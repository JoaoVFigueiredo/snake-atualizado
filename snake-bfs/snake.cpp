...
/* Cobra */
#define SYM_HEAD "\xF0\x9F\x94\xB4"      // 🔴 vermelho
#define SYM_HEAD_AUTO "\xF0\x9F\x9F\xA3" // 🟣 roxo
#define SYM_BODY "\xF0\x9F\x9F\xA6"      // 🟦 azul
#define SYM_TAIL "\xF0\x9F\x9F\xAA"      // 🟪 roxo
#define SYM_BODY_FAST "\xF0\x9F\x9F\xA7" // 🟧 laranja
...
#define C_RUNNING    10
#define C_PAUSED     12
#define C_BG         1   // azul escuro (estilo noite)
#define C_TITLE      14
#define C_SELECT     14
#define C_NORMAL     7
#define C_FLASH      15
...
#define C_BORDER_L1  4   // vermelho
#define C_BORDER_L2  5   // roxo
#define C_BORDER_L3  6    // amarelo escuro
#define C_BORDER_L4  3   // ciano
#define C_BORDER_L5  1   // azul escuro
...
int find_nearest_fruit_bfs(void) {
    ...
    if (best_idx >= 0) {
        ...
        memcpy(bfs_visited_cells, best_visited, sizeof(BfsNode) * bfs_visited_count);
        return 1;
    }
...
void input_menu(GameState *state) {
    ...
    if (k == 0 || k == 224) {
        k = _getch();
        if      (k == 72) { menuIndex--; if (menuIndex < 0) menuIndex = 3; }
        else if (k == 80) { menuIndex++; if (menuIndex > 3) menuIndex = 0; }
        return;
    }
    ...
    else if (k == '4' || k == 27) { *state = STATE_EXIT; }
    else if (k == 13) {
